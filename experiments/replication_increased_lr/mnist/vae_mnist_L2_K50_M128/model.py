class mnist2_model(nn.Module):
    ...
    def compute_loss_for_batch(self, data, model, K=K, testing_mode=False, alpha=alpha):
        ...
        log_p_z = compute_log_probabitility_gaussian(
            z2,
            torch.zeros_like(z2, requires_grad=False),
            torch.zeros_like(z2, requires_grad=False),
        )
        ...
        if discrete_data:
            ...
            log_p_2 = compute_log_probabitility_gaussian(
                recon, data_k_vec, torch.zeros_like(recon)
            )
        ...
        if model_type == "iwae" or testing_mode:
            ...
            log_w_matrix = (
                (log_p_z + log_p_1 + log_p_2 - log_q_1 - log_q_2).view(B * K, 1) * 1 / K
            )
            return 0, 0, 0, -torch.sum(log_w_matrix)
        elif model_type == "general_alpha" or model_type == "vralpha":
            log_w_matrix = (log_p_z + log_p_1 + log_p_2 - log_q_1 - log_q_2).view(
                -1, K
            ) * (1 - alpha)
        elif model_type == "vrmax":
            log_w_matrix = (
                (log_p_z + log_p_1 + log_p_2 - log_q_1 - log_q_2)
                .view(-1, K)
