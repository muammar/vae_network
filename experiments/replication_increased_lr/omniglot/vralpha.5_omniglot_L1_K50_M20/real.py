...
def compute_log_probabitility_gaussian(obs, mu, logstd, axis=1):
    ...
    return torch.sum(
        -0.5 * ((obs - mu) / (torch.exp(logstd) + torch.tensor(1e-19))) ** 2 - logstd,
        axis,
...
def train(round_num, epoch, optimizer):
    ...
    for batch_idx, [data] in enumerate(train_loader):
        ...
        if batch_idx % log_interval == 0:
            logging.info(
                "Round {}, Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                    round_num,
                    epoch,
                    batch_idx * len(data),
...
train_loader = DataLoader(
    TensorDataset(data_train_t), batch_size=batch_size, shuffle=True
)
test_loader = DataLoader(
    TensorDataset(data_test_t), batch_size=test_batch_size, shuffle=True
)
