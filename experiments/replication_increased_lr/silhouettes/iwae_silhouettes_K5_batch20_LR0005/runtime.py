...
for r in range(num_rounds + 1):
    ...
    for epoch in range(2**r):
        train(r, epoch)
    with open(
        f"{model_type}_{data_name}_K{K}_M{batch_size}/{model_type}_{data_name}_K{K}_M{batch_size}_LR0005.pt",
        "wb",
    ) as f:
        print(datetime.datetime.now())
        logging.info(datetime.datetime.now())
...
_test(r, epoch)
with open(
    f"{model_type}_{data_name}_K{K}_M{batch_size}/{model_type}_{data_name}_K{K}_M{batch_size}_LR0005_{r}_{epoch}.pt",
    "wb",
) as f:
    print(datetime.datetime.now())
    logging.info(datetime.datetime.now())
