...
train_loader = DataLoader(
    TensorDataset(data_train_t), batch_size=batch_size, shuffle=True, pin_memory=True
)
test_loader = DataLoader(
    TensorDataset(data_test_t),
...
for r in range(num_rounds + 1):
    ...
    print(
        f"====== About to train for {3**r} epochs in round {r} with learning rate {round(current_round_lr,7)}========"
    )
    logging.info(
        f"====== About to train for {3**r} epochs in round {r} with learning rate {round(current_round_lr,7)}========"
    )
    ...
    with open(
        f"{model_type}_{data_name}_K{K}_M{batch_size}/{model_type}_{data_name}_K{K}_M{batch_size}.pt",
        "wb",
    ) as f:
        print(datetime.datetime.now())
        logging.info(datetime.datetime.now())
    ...
    with torch.no_grad():
        ...
        save_image(
            ...
            f"{model_type}_{data_name}_K{K}_M{batch_size}/samples/sample_"
            + str(r)
            + "_"
            + str(epoch)
            + ".png",
...
_test(r, epoch)
with open(
    f"{model_type}_{data_name}_K{K}_M{batch_size}/{model_type}_{data_name}_K{K}_M{batch_size}_{r}_{epoch}.pt",
    "wb",
) as f:
    print(datetime.datetime.now())
    logging.info(datetime.datetime.now())
